...
- Porte trasversali iniettate ovunque e relative implementazioni: `IClock` → `SystemClock` (runtime) / `FixedClock` (test e demo); `IIdGenerator` → `UuidIdGenerator` / `SequentialIdGenerator` (test); `ILogger` → `ConsoleLogger` / `NoopLogger` (test); `IEventBus` → `InProcessEventBus`; `IMediaStorage` (`put`/`read`/`getUrl`/`delete`) → `MediaStorageLocalDisk` (disco, predefinito) / `MediaStorageMock` (memoria, test), entrambe verificate dalla stessa suite `tests/contracts/media-storage.contract.ts`. Fuori dalle implementazioni non esistono `Date.now()`, `Math.random()` o `crypto` diretti. I tipi condivisi fra `config/env.ts` e i mock (`ProviderKind`, `RepositoryProvider`, `InfinityMockMode`, `CrmMockMode`, `MOCK_PHONE_RULES`) vivono in `services/interfaces/provider-kinds.ts` e `services/interfaces/mock-config.ts`, così la configurazione non importa mai una classe mock.
...
└── src/
    ...
    ├── proxy.ts                       # [M1/M5 fatto] guard JWT (firma e scadenza) su /accettazione, /check-in (e il vecchio /tablet), /cambia-password, /sistema, /manager, /admin, /api/v1 (esclusi auth/, health, public/, webhooks/); redirect /login?next=; x-correlation-id. Rate limit e SYNC_SECRET rinviati
    ...
    │   ├── (auth)/cambia-password/page.tsx # [M8 fatto] cambio password (obbligato dopo creazione o reset, o volontario); ChangePasswordForm
    │   ├── (auth)/login/page.tsx      # [M1 fatto] login operatore: credenziali, Sportello/Brand (chip marchi), Postazione; account demo cliccabili
    │   ├── (operator)/                # area autenticata: accettazione, tablet, manager, admin, sistema (i KIOSK vengono rimandati al tabellone)
    ...
    │   │   ├── accettazione/archivio/page.tsx     # [M8 fatto] archivio ispezioni: ricerca per targa o codice, schede con foto per categoria, segnaposto per i file oltre la retention
    │   │   ├── admin/page.tsx         # [M8 fatto] pannello amministratore: operatori (crea, modifica, disattiva, reset password) e assistenza (accettazioni occupate, pratiche in carico)
    ...
    │   │   ├── tablet/page.tsx        # [M8] redirect a /check-in (conserva `?pratica=`): il vecchio indirizzo dei segnalibri
    │   ├── (checkin)/                 # [M8 fatto] check-in veicolo a tutto schermo: layout proprio (sessione, niente shell), error.tsx dedicato
    │   │   └── check-in/page.tsx      # CheckInQueue: barra minima, due schede grandi, CheckInScreen; `?pratica=<id>` apre subito il giro foto (arrivo dalla dashboard)
    ...
    │       ├── auth/me/route.ts, auth/workstation/route.ts    # me [M1 fatto] (consentita anche con password provvisoria); cambio postazione senza logout rinviato (M1-T11-S02)
    │       ├── auth/change-password/route.ts                   # [M8 fatto] POST cambio password: verifica l'attuale (rate limit per utente), rinnova il cookie senza l'obbligo
    ...
    │       ├── system/cron/media-retention/route.ts  # [M8 fatto] POST retention foto per cron esterno (sessione ADMIN o x-cron-secret): file scaduti eliminati, record archiviati
    │       ├── admin/operators/route.ts              # [M8 fatto] GET elenco operatori + sportelli/postazioni, POST creazione (solo ADMIN)
    │       ├── admin/operators/[id]/route.ts         # [M8 fatto] PATCH nome, ruolo, sportelli, postazione, attivo/inattivo (divieti su sé stessi e ultimo admin)
    │       ├── admin/operators/[id]/reset-password/route.ts # [M8 fatto] POST password provvisoria (mostrata una volta)
    │       ├── admin/assistance/route.ts             # [M8 fatto] GET accettazioni occupate e pratiche in carico con minuti trascorsi
    │       ├── inspections/archive/route.ts          # [M8 fatto] GET archivio ispezioni ?q= (targa o codice), area accettazione
    ...
    │   ├── inspection-media/          # E – [M5/M8 fatti] CheckInQueue (barra minima, due schede grandi, avviso da PC), CheckInScreen (testata fissa con codice e targa, comandi fissi in basso, annotazioni rapide, chiusura bloccata senza le 4 foto), PhotoCapture (slot numerati per parte del veicolo, icone a tratto, fotocamera + anteprima in caricamento), MediaGallery (note e foto raggruppate per categoria, ingrandimento a tutto schermo); UploadQueue da fare
    │   ├── admin/                     # [M8 fatti] AdminDashboard, OperatorsPanel (tabella, dialog crea/modifica, password provvisoria), AssistancePanel (schede accettazioni, rimetti in coda, annulla a due tocchi)
    ...
    ├── hooks/                         # [fatti] useQueue (3 s), useAppointmentActions (con onSuccess per il flusso responsive), usePublicStatus (5 s), useBayDisplay e useWaitingBoard (2 s, scollegato dopo 3 tentativi), useBdcLeads (10 s), useMediaQuery/useIsTouchLayout/useTouchLayoutKind (`(pointer: coarse)`: il dispositivo, non la larghezza), useLiveUpdates (SSE + invalidazione query, polling come rete di sicurezza)
    ...
    │   ├── entities/                  # brand, desk, workstation, workstation-claim (accettazione occupata da un operatore collegato), bay, operator, customer, vehicle, appointment, notification, media-asset, sync-run, crm-outbox-event
    ...
    │   └── index.ts
    ├── infrastructure/                # [M8 fatto] adapter reali verso sistemi esterni (importati solo dai factory)
    │   ├── adapters/infinity/         # infinity-odbc-config.ts (DSN, sorgente del planning, sede), OdbcClient.ts (IOdbcClient, SqlAnywhereOdbcClient), infinity-planning-query.ts (SQL della procedura nativa sp_off_docs_planning e delle tabelle tdo_pre, arricchimenti clienti/mdm_pre_inc/vs_off_inc_tipoinc, InfinityPlanningRecord → InfinityAgendaDto), InfinityServiceOdbc (porta IInfinityService, sola lettura, ripiego automatico, checkAccess con GRANT suggeriti)
    │   └── messaging/spoki/           # spoki-config.ts (template kind, payload snake_case), SpokiClientAdapter (simulation | live), SpokiService (porta ISpokiService), SpokiActivityLog (registro in memoria)
    ...
    │   ├── portal/                    # B – [M8 fatto] CustomerPortalService (stato per targa o token, tappe del percorso, auto-segnalazione ritardo), portal-token.ts (HMAC per pratica, solo server)
    │   ├── notifications/             # C – [M3 fatti] NotificationOrchestrator (WhatsApp → SMS → contatto manuale; sendReminders), templates.ts, AppointmentReminderService (promemoria giorno prima con sync anticipata di domani e giorno stesso; lanciati dallo scheduler o dal cron), CustomerMessagingPolicy (eventi del bus → conferma inserimento, turno vicino, annullamento; lavoro fuori dal chiamante; dry-run dichiarato quando Spoki è bloccato)
    ...
    │   ├── auth/                      # [M1/M8 fatti] IAuthService, LocalAuthService (account locali + JWT HS256 con jose, riverifica operatore, cambio password, occupazione dell'accettazione al login e rilascio al logout), session-token.ts, workstation-availability.ts (accettazioni libere vs occupate per la pagina di login)
    ...
    │   ├── admin/                     # [M8 fatti] OperatorAdminService (elenco, creazione, modifica, reset password; divieti su sé stessi e ultimo ADMIN) e AssistanceService (accettazioni occupate e pratiche in carico)
    │   └── media/                     # [M5/M8 fatti] InspectionArchiveService (ricerca per targa/codice, purgeExpired: file eliminato, record archiviato) e InspectionService: addPhoto (validazione tipo/dimensione, IMediaStorage, MediaAsset), listPhotos, completeCheckIn (note salvate prima della chiusura, poi CRM)
    ...
    │   ├── constants.ts               # TIMEZONE, SYNC_HOUR_LOCAL, CODE_PREFIX, DEFAULT_MEDIA_DIR, BAY_COUNT, POLLING_MS, STALE_WARNING_MS, PUBLIC_STATUS_RATE_LIMIT, RELEASING_DISPLAY_MS, LATE_GRACE_MINUTES, MAX_SKIPS_BEFORE_ANOMALY
...
| **Operator** (accettatore) | `username`, `displayName`, `role: ADVISOR \| SUPERVISOR \| ADMIN \| KIOSK`, `deskIds`, `defaultWorkstationId`, `passwordHash`, `isActive`, `mustChangePassword` | SUPERVISOR (a schermo "Manager"): forza stato, riapre NO_SHOW, annulla pratiche in carico, conferma fallback manuali. ADMIN: pagina Sistema e pannello `/admin` (utenti, assistenza). KIOSK: account dei dispositivi, solo tabellone. Ruolo riverificato lato server. |
...
| **MediaAsset** | `kind: PHOTO \| VIDEO`, `category`, `mimeType`, `sizeBytes`, `storageKey`, `thumbnailKey`, `capturedByOperatorId`, `capturedAt`, `expiresAt`, `archivedAt` | Il binario vive dietro `IMediaStorage`; alla scadenza (`PHOTO_RETENTION_DAYS`) il file viene eliminato e il record resta con `archivedAt`; trascorsi `PHOTO_HARD_DELETE_DAYS` dall'archiviazione anche il record viene eliminato (ADR-022). |
...
stateDiagram-v2
  ...
  COMPLETED --> IN_PROGRESS : Riapri pratica (errore o check-in da rifare, solo oggi)
...
**Password provvisoria e cambio obbligatorio (M8).** `Operator.mustChangePassword` è true alla creazione dal pannello e dopo ogni reset; `LocalAuthService.issue` lo copia nel claim del JWT, così il proxy (che non ha il container) rimanda a `/cambia-password` ogni pagina e risponde 403 `PASSWORD_CHANGE_REQUIRED` a ogni API, salvo la pagina stessa e le rotte `auth/`. `LocalAuthService.verify` rilegge il flag dal repository: un reset fatto mentre l'operatore è collegato vale alla prima richiesta successiva, tramite `requireSession` (redirect) e `readApiSession` (401, salvo `allowPendingPasswordChange`). `changePassword` verifica la password attuale, impone `MIN_PASSWORD_LENGTH` e la diversità, e riemette il token con il flag a false. Il client, su 403 con quel codice, porta l'operatore alla pagina di cambio; a cambio riuscito fa una navigazione completa verso `destinationAfterPasswordChange(next, role)` (mai il cambio password o il login stesso).

**Un'accettazione, un operatore collegato (M8-T05).** Al login `LocalAuthService` registra un `WorkstationClaim` (postazione, operatore, scadenza = scadenza della sessione) e rifiuta con `VALIDATION` la scelta di un posto occupato da un altro operatore; un operatore che rientra o si sposta lascia il posto precedente; `logout(session)` libera solo il proprio; una sessione scaduta libera il posto da sola. La pagina di login calcola con `workstationAvailability` le accettazioni libere (né occupazione attiva, né veicolo in carico nella campata) e propone solo quelle, elencando le occupate con il motivo. Il record è volatile (vive quanto una sessione) e non entra nello snapshot.
...
| riapri pratica | `reopen-completed` / `COMPLETED → IN_PROGRESS` | "Completato" premuto per errore o check-in da rifare; in carico a chi riapre, campata libera se c'è; solo nella giornata corrente |
| chiusa d'ufficio | `autoClosedAt` su `COMPLETED` | Pratica ancora in carico alla chiusura automatica: completata dal sistema, "da confermare" finché un responsabile non conferma (`confirm-auto-close`, CRM avvisato) o un operatore non riapre |
| nuovo cliente / inserimento manuale | `ManualIntakeService.create` / `source: MANUAL` | Cliente senza appuntamento: pratica nella coda di oggi con il prossimo codice della sequenza della sync, sportello che serve il marchio, orario atteso adesso |
...
| **017** Un'unica applicazione che cambia comportamento con il dispositivo, non due app | `useIsTouchLayout()` su `(pointer: coarse)` (dal 2026-09-14; prima ≤ 1024 px, che scambiava un iPad in orizzontale per un monitor): sul tablet "Prendi in carico" porta all'ispezione e il dettaglio è una finestra centrale con il passaggio al check-in; sul PC apre il pannello laterale, senza alcun pulsante di check-in, e `/check-in` mostra solo un avviso; la destinazione è decisa nell'`onSuccess` dell'azione | Applicazione tablet separata (due basi di codice, due sessioni, due volte i bug); stesso identico flusso ovunque (al banco porterebbe fuori dalla coda senza motivo) | Una sola coda, una sola sessione, un solo insieme di regole; il dispositivo cambia solo la scorciatoia, mai quello che il sistema permette di fare |
...
| **022** Retention delle foto: il file scade, la scheda resta (per un periodo, poi sparisce anche lei) | `MediaAsset.expiresAt` calcolato allo scatto (`PHOTO_RETENTION_DAYS`, default 30); `InspectionArchiveService.purgeExpired` elimina il binario e scrive `archivedAt`; l'archivio mostra "file eliminato" con la data; secondo passaggio: record archiviati da oltre `PHOTO_HARD_DELETE_DAYS` (90) eliminati, contando dall'archiviazione | Cancellare anche il record (si perde la prova che il giro era stato fatto); tenere i file per sempre (spazio e GDPR) | Al ritiro il cliente contesta un danno anche mesi dopo: serve sapere *che* le foto c'erano e *quando*, non conservarle a tempo indeterminato |
| **023** Gestione utenti nel dominio, con divieti che il server applica da solo | `OperatorAdminService`: nome utente minuscolo e unico, password hashata, ruolo `KIOSK` per i dispositivi, vietato disattivare/degradare sé stessi e l'ultimo ADMIN attivo; password provvisoria da CSPRNG con alfabeto dettabile, mostrata una volta | Modificare il seed a mano (richiede un deploy); affidare i divieti alla UI (una chiamata diretta li aggira) | L'officina deve poter fare onboarding e uscite senza un tecnico, e non deve mai restare senza un amministratore che possa entrare |
| **024** Colore come significato: verde = avanzamento, blu = struttura | `bg-brand-primary` (verde) solo su completamento e avanzamento; `bg-brand-secondary` (blu) su navigazione, presa in carico e azioni secondarie; `Button` `default` blu, `success` verde | Verde ovunque come "colore del marchio" (ogni pulsante urla) | Sul piazzale l'occhio deve trovare in un attimo l'azione che chiude il lavoro; il resto deve stare in secondo piano |
| **025** L'obbligo di cambiare la password provvisoria vive nel dominio e nel token, non nella UI | `Operator.mustChangePassword` (true alla creazione e al reset), claim nel JWT letto dal proxy, riverifica dal repository in `verify`; una sola pagina raggiungibile e 403 `PASSWORD_CHANGE_REQUIRED` sulle API | Chiedere il cambio "gentilmente" con un banner (chi detta la password potrebbe usarla per giorni); far scegliere la password all'amministratore (nessuno deve conoscere quella di un collega) | La password provvisoria la conoscono in due: finché non è sostituita, l'account non deve poter fare nulla che valga la pena di attribuire a qualcuno |
| **026** Un'accettazione, un operatore collegato: l'occupazione si registra al login e la applica il server | `WorkstationClaim` in `IWorkstationClaimRepository` (una per postazione, una per operatore, scade con la sessione); `LocalAuthService.login/switchWorkstation` rifiutano il posto occupato da un altro; `logout` libera il proprio; la pagina di login propone solo le libere e mostra le occupate con il motivo (collega collegato o veicolo in carico) | Solo filtro nella UI (due login contemporanei passerebbero entrambi); registro delle sessioni completo lato server (la sessione resterebbe un JWT e servirebbe una revoca che oggi non c'è) | I monitor chiamano il cliente a un'accettazione fisica: due operatori sullo stesso posto manderebbero due clienti nello stesso punto; la scadenza automatica evita che un posto resti "occupato" da chi ha chiuso il browser senza uscire |
| **027** Il check-in veicolo è un'app a tutto schermo dentro la stessa applicazione | Gruppo di rotte `(checkin)` con layout senza shell; barra minima disegnata dalla vista (coda, esci); testata e comandi fissi, corpo scorrevole; manifest PWA con `start_url: /check-in` e `display: standalone` | Riusare la shell dell'area operatore nascondendo l'intestazione via CSS (menu e orologio restano nel DOM e nel peso della pagina); un'app nativa separata (due basi di codice, ADR-017) | Sul piazzale il tablet deve fare una cosa sola e sembrare fatto per quella; la sessione, i permessi e le API restano gli stessi della dashboard |
| **028** Una pratica completata si riapre, ma solo in carico e solo oggi | `COMPLETED → IN_PROGRESS` nella state machine; `reopenCompleted` assegna una campata libera, conserva la presa in carico originale, azzera la chiusura; la chiave CRM del check-in include l'istante di completamento, così il secondo check-in non viene scartato come doppione | Lasciare `COMPLETED` terminale (un errore di tocco costa una telefonata al CRM e un dato KPI sbagliato); riaprire verso `WAITING` (il cliente non è tornato in coda: è ancora lì con l'auto) | Il tasto sbagliato è la normalità del banco; correggerlo deve costare un tocco, non una richiesta al reparto IT |
| **029** Alla chiusura automatica le pratiche in carico si completano "d'ufficio", non si annullano | `autoClosedAt` + `autoCloseConfirmedAt` su `Appointment`; `isAutoClosedPending`; conferma del responsabile con check-in al CRM, oppure riapertura; badge in riga, dettaglio e CSV; il monitor non mostra RELEASING per queste chiusure | Annullarle (cancella lavoro fatto: alle 19 una pratica in carico è quasi sempre un veicolo accettato senza il tocco finale); un nuovo stato `AUTO_CLOSED` (ogni switch dell'applicazione andrebbe toccato per un caso che vale poche pratiche al giorno) | I numeri che direzione e BDC leggono la mattina dopo devono raccontare cosa è successo davvero, e la decisione finale spetta a una persona |
| **030** L'inserimento manuale usa la stessa sequenza di codici e lo stesso evento della sync | `ManualIntakeService`: `codeGenerator.next` (stesso contatore giornaliero), `APPOINTMENT_CREATED` con `source: MANUAL` (la policy manda la conferma con il codice), targa già in coda rifiutata, orario atteso = adesso, modello `n/d` | Una coda separata per i walk-in (i monitor chiamerebbero due numerazioni); creare la pratica direttamente in carico (salterebbe la fila di chi era prenotato) | Il cliente senza appuntamento è un cliente come gli altri: entra in coda con un numero che i monitor sanno chiamare, dietro a chi era arrivato prima |
| **031** Spoki reale con una modalità di simulazione che non parla con la rete | `SPOKI_MODE=simulation` (predefinito): `SpokiClientAdapter` registra il payload nel log e in `SpokiActivityLog`, risponde 200 e la consegna risulta DELIVERED; `live` chiama le automazioni con `SPOKI_API_KEY`; in simulazione il segreto di sviluppo e il seed demo restano ammessi (`isAllMock`, `assertNoDemoCredentialsOutsideMock`) | Testare direttamente in live con un numero interno (ogni prova costa crediti e disturba un telefono vero); un secondo mock "quasi reale" (due codici da tenere allineati) | Il codice che andrà in produzione è lo stesso che gira oggi: cambia solo un interruttore, e prima di girarlo si leggono i payload esatti nel pannello |
| **032** Il ritardatario riattivato entra in coda "adesso": dopo i presenti, prima degli attesi | `reactivate`: NO_SHOW → WAITING con `rescheduledAt = now`; `compareQueueOrder` (orario effettivo, poi sequenza) è l'unica regola d'ordine di dashboard, tablet e portale; il codice non cambia; il lead del BDC si chiude con nota | Rimetterlo al suo orario originale (supererebbe chi è arrivato puntuale); metterlo in fondo assoluto (dopo chi è atteso fra due ore); riassegnare il codice (i monitor e il cliente lo conoscono già) | Chi arriva tardi non deve scavalcare chi era puntuale, ma non deve nemmeno aspettare chi non è ancora arrivato |
| **033** Infinity si legge direttamente dal database via ODBC, in sola lettura, scegliendo il DSN dall'ambiente | `InfinityServiceOdbc` su SQL Anywhere 12 con il driver già installato sul server; `INFINITY_ODBC_DSN` (`Infinity02` → `Infinity01`) e `INFINITY_DB_TYPE`; solo `SELECT` parametriche; due sorgenti con le stesse colonne, la procedura nativa `sp_off_docs_planning` (quella del planning di Infinity, richiede `GRANT EXECUTE`) e le tabelle `tdo_pre` e collegate, scelte con `INFINITY_PLANNING_SOURCE=auto` (procedura se concessa, altrimenti tabelle con un avviso); nome cliente dalla vista `clienti`, lavorazioni da `mdm_pre_inc`, stato da `off_stati_doc`; il DTO è lo stesso del mock; `checkAccess` elenca i permessi mancanti; il modulo nativo `odbc` si carica solo alla prima query e resta fuori dal bundle (`serverExternalPackages`) | API REST di Infinity (non esposta per il planning); export file periodico (latenza e un processo in più da presidiare); copiare la query nativa così com'è (dipende dalla sessione dell'utente Infinity per il colore della riga); scrivere su Infinity da subito (rischio sul gestionale, fuori perimetro) | Il DSN è l'unico punto di contatto già presidiato dall'IT: cambiare database è cambiare una variabile; la sola lettura rende l'errore peggiore un'agenda vuota, mai un dato sporco nel gestionale; il ripiego sulle tabelle fa funzionare l'app anche prima che i GRANT arrivino |
| **034** Spoki con doppio interruttore e blocco di sicurezza predefinito; due soli promemoria | `SPOKI_SAFETY_LOCK=true` di default: la chiamata HTTP parte solo con `SPOKI_MODE=live` e blocco tolto, altrimenti l'adapter formatta il payload (formato Spoki con `secret` per automazione e `custom_fields`), lo registra con il motivo del blocco e risponde 200; `AppointmentReminderService` manda il giorno prima (anticipando la sync di domani, così il codice esiste già) e il giorno stesso, alle ore configurate, idempotenti per pratica e giornata; il test manuale del pannello accetta solo numeri digitati a mano e rifiuta quelli dei clienti in agenda | Un solo interruttore `SPOKI_MODE` (un errore di configurazione basta a scrivere ai clienti); leggere l'agenda di domani senza persisterla (nessun codice da comunicare); mandare il promemoria del mattino dentro la sync delle 06:00 (telefoni che suonano all'alba) | Nella fase di prova nessun cliente deve ricevere nulla per sbaglio: servono due decisioni esplicite per sbloccare; il codice nel messaggio del giorno prima richiede che la pratica esista già |
| **035** Il portale cliente si apre senza login, con la targa o con un token HMAC per pratica, e concede una sola azione | `CustomerPortalService` (`application/portal`): stato con tappa del percorso, coda, accettatore e sede; token = HMAC-SHA256(segreto di sessione, id pratica) troncato, calcolato al volo e messo nel link dei messaggi (`/portal?targa=…&t=…`), nessuna colonna in più; "Sto arrivando in ritardo (+10 min)" scrive `customerLateNoticeAt`/`customerEtaAt`, pubblica `CUSTOMER_LATE_NOTICE` con attore `CUSTOMER` e non tocca l'ordine della coda; pratiche concluse da oltre 24 ore o di giornate passate → `expired` | Login o OTP per il cliente (attrito inutile per leggere una coda); token salvato sulla pratica (migrazione e sincronizzazione per un dato derivabile); spostare la pratica in coda quando avvisa il ritardo (scavalcherebbe chi è puntuale) | Il cliente deve leggere il proprio stato in un tocco dal telefono; il token protegge il link WhatsApp dall'enumerazione, la targa resta la via del QR con i limiti di frequenza; l'accettatore decide, il cliente avvisa |
...
| **M6** Modulo F CRM/BDC + hardening | Modulo F | **[cruscotto BDC fatto]** `BdcLeadService`, `/api/v1/crm/leads`, `modules/crm` e `/manager`; restano `AnomalyReporter`, svuotamento outbox con attesa progressiva, `/api/v1/crm/outbox`, SSE `/api/v1/events`, log JSON, `RUNBOOK_OPERATIVO.md`, Dockerfile standalone | No-show → evento outbox consegnato al `CrmServiceMock` e lead nel cruscotto BDC, chiudibile con "Segna come ricontattato"; con `MOCK_CRM_MODE=error` resta PENDING/FAILED, ritentato, chiudibile a mano; nessuna azione operatore bloccata dal CRM |
...
L'audit finale del 2026-09-14 (`docs/AUDIT_PRODUZIONE.md`) elenca, in ordine di gravità, cosa manca per la produzione e propone una milestone M9 "Pilota" (inserimento manuale, persistenza Prisma/PostgreSQL, Docker+CI+log JSON, adapter Infinity in lettura) prima del passaggio completo ai servizi reali. Il 2026-09-15 l'inserimento manuale è fatto e l'adapter Infinity in lettura esiste ed è stato provato sul database `infinity02` (ADR-033, `docs/INFINITY_ODBC.md`); restano persistenza e confezionamento.
