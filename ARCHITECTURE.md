...
| --- | --- | --- | --- |
| Runtime / package manager | Node.js LTS + pnpm | Node 24 LTS (minimo 22, `engines.node >=22`), pnpm 10.x | Node 24 è la LTS attiva; `fetch`, `AbortSignal.timeout`, `crypto.randomUUID` nativi. pnpm: install deterministici e `node_modules` rigoroso (intercetta import fantasma che violerebbero il layering). Deploy come singolo processo Node long-running (Docker/VM/servizio Windows), **mai serverless** finché lo stato vive in memoria. Tutte le versioni di questa tabella vanno verificate e pinnate in M0-T06 (annotare la data nel `package.json`). |
| Framework | Next.js App Router: Server Components per il primo render, Route Handlers `/api/v1` per letture in polling e mutazioni, `output: 'standalone'` | 16.x (fallback 15.5) | Baseline del PO. Una sola codebase per dashboard, portale, display, tablet e API. In Next 16 `middleware.ts` si chiama `src/proxy.ts`; `cookies()`/`headers()`/`params` sono async. Le mutazioni di dominio passano da Route Handlers, non da Server Actions. |
| Linguaggio | TypeScript strict (`strict`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`, `verbatimModuleSyntax`, `noUnusedLocals`, `noUnusedParameters`, `noImplicitReturns`, `noFallthroughCasesInSwitch`) | 5.9.x (major 5 pinnato: il passaggio a TS 6/7 si valuta quando il plugin Next e typescript-eslint lo supportano) | Regola CLAUDE.md. Branded ID, `Result<T,E>` e unioni discriminate rendono irrappresentabili gli stati illegali e fanno verificare dal compilatore lo swap Mock→Real. Scaffold verificato con `npx -y -p typescript@5 tsc --noEmit`. |
| UI | React 19 + Tailwind CSS 4 (config CSS-first) + shadcn/ui (Radix, componenti copiati in `src/components/ui`) + lucide-react | React 19.x, Tailwind 4.x, shadcn CLI 3.x | Baseline del PO. Nessun lock-in (codice nel repo), primitive accessibili (Table, Dialog, Toast, Badge) per una dashboard monopagina densa e target touch da 48 px su tablet e kiosk. |
...
├── .gitignore                         # Node/Next: node_modules, .next, .env*, .data/, coverage   [SCAFFOLD]
├── .editorconfig                      # UTF-8 senza BOM, LF, 2 spazi (M0)
├── .env.example                       # SERVICES_PROVIDER=mock, interruttori MOCK_*, APP_TIMEZONE (M0)
├── tsconfig.json                      # strict, noUncheckedIndexedAccess, noUnused*, alias "@/*" -> src/*   [SCAFFOLD]
├── package.json / pnpm-lock.yaml      # (M0) pnpm, script dev/build/test/lint/typecheck
├── next.config.ts                     # (M0) output: 'standalone'
├── postcss.config.mjs                 # (M0) Tailwind 4
...
└── src/
    ...
    ├── instrumentation.ts             # (M1) bootstrap container + scheduler (sync 06:00 con catch-up, svuotamento outbox)
    ├── app/                           # SOLO routing e composizione pagine, nessuna logica. Nello scaffold contiene solo .gitkeep:
    │   │                              # le sottocartelle (route group, api/v1) nascono con il bootstrap Next.js (M0-T11) e le milestone indicate
    │   ├── layout.tsx                 # root layout, QueryClientProvider, Toaster
    │   ├── globals.css                # Tailwind 4 + token colori stati
    │   ├── page.tsx                   # redirect → /accettazione o /login
    ...
    │       └── health/route.ts                       # (M0) liveness + HealthStatus aggregato delle quattro porte esterne
    ...
    │   ├── env.ts                     # EnvSource via globalThis (nessun @types/node), parseEnv() con default sicuri, APP_TIMEZONE validato
    ...
    │   ├── dates.ts                   # businessDate Europe/Rome via Intl, parsing orari   [SCAFFOLD]
...
- **Encoding**: UTF-8 senza BOM, fine riga LF (`.editorconfig` + Prettier `endOfLine: 'lf'`); su Windows scrivere con strumenti che rispettano l'encoding e verificare con un controllo byte prima del commit.
...
| --- | --- | --- | --- |
| **M0** Bootstrap Next.js e tooling | Prerequisito | `TASKS.md` (M0-T03), `package.json` pnpm con dipendenze pinnate, Tailwind 4 + shadcn, ESLint 9 con `no-restricted-imports`, Vitest, Zod al posto dei type guard manuali, App Router minimo con `/api/v1/health`, README, GLOSSARIO, ADR, CI | `pnpm dev` mostra la pagina base; `/api/v1/health` risponde con le quattro porte esterne UP (mock); typecheck, lint, test e build verdi in CI |
