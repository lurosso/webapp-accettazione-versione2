...
| **MediaAsset** | `kind: PHOTO \| VIDEO`, `category`, `mimeType`, `sizeBytes`, `storageKey`, `thumbnailKey`, `capturedByOperatorId`, `capturedAt`, `expiresAt`, `archivedAt` | Il binario vive dietro `IMediaStorage`; alla scadenza (`PHOTO_RETENTION_DAYS`) il file viene eliminato e il record resta con `archivedAt` (ADR-022). |
...
| **022** Retention delle foto: il file scade, la scheda resta | `MediaAsset.expiresAt` calcolato allo scatto (`PHOTO_RETENTION_DAYS`, default 30); `InspectionArchiveService.purgeExpired` elimina il binario e scrive `archivedAt`; l'archivio mostra "file eliminato" con la data | Cancellare anche il record (si perde la prova che il giro era stato fatto); tenere i file per sempre (spazio e GDPR) | Al ritiro il cliente contesta un danno anche mesi dopo: serve sapere *che* le foto c'erano e *quando*, non conservarle a tempo indeterminato |
